const RESULTS_RECIPIENT = 'contact@spathiswellbeing.com';
...
export default async function handler(req, res) {
  ...
  if (consent !== 'yes') {
    return res.status(400).json({ error: 'Consent is required to send results' });
  }

  if (!['grounded', 'unfolding', 'wayforward'].includes(pathway)) {
    return res.status(400).json({ error: 'Invalid pathway result' });
  }

  const safeName = escapeHtml(name || 'Not provided');
  const safeEmail = escapeHtml(email || 'Not provided');
  const safePhone = escapeHtml(phone || 'Not provided');
  const resultName = pathwayName(pathway);

  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;color:#1B1B1A;line-height:1.5;max-width:720px;margin:auto;">
      <h2 style="font-weight:500;">New Via Method Mapping result</h2>
      <p><strong>Recommended pathway:</strong> ${escapeHtml(resultName)}</p>
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        reply_to: RESULTS_RECIPIENT,
        subject: `Via Method result — ${resultName}${name ? ` — ${name}` : ''}`,
        html
      })
    });
    ...
    return res.status(500).json({ error: 'Unable to send result email' });
  }
}
